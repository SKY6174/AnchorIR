...
앵커 사업의 효율적인 기획 및 성과관리를 위해 본 대시보드는 **프로젝트 - 단위과제 - 추진전략 - 전략과제 - 프로그램**의 5단계 고유 연계 체계를 도입하여 관리하고 있습니다.

```mermaid
graph TD
    PJ["1단계: PJ (프로젝트)<br/>울산시 제시 4대 핵심 분야"] 
    --> UP["2단계: UP (단위과제)<br/>목표 달성 12대 단위사업 (A1~D3)"] 
    --> S["3단계: S (추진전략)<br/>단위과제 달성 거시 비전"] 
    --> T["4단계: T (전략과제)<br/>전략 실현 고유 중점 분야"] 
    --> PG["5단계: PG (프로그램)<br/>예산 및 KPI 매칭 실천 단위"]
```

#### ⚙️ 프로그램 ID 규칙 (Rule)
* **형식**: `단위과제번호 - (추진전략번호 + 전략과제번호) - 프로그램번호`
* **예시**: `A1가-S1T1-1`
  * 단위과제: **A1가** (지역과 미래를 만드는 UC-HYPER 전문기술인재양성)
  * 추진전략: **S1** (지역인력 수요분석 기반 UC-HYPER 실무인재 양성)
  * 전략과제: **T1** (UC-HYPER 기반 주문식 교육과정 및 혁신 교수법 개발)
  * 프로그램: **1번 프로그램** (`UC-HYPER 교수법 개발(공학/비공학)`)
* *액션플랜(Action Plan; AP): 각 프로그램 수행을 위해 예산(본사업비/이월비), 담당자, 추진 단계, 마일스톤 기한 등을 상세히 테이블로 명시한 최하위 실천 명세입니다.

#### 🗂️ 과제 & 전략 내비게이터 및 매칭 예시
대시보드 좌측의 **과제 & 전략 내비게이터**에서 각 단계를 선택하면 우측 패널에 연계된 정보가 매칭되어 표시됩니다.

| 내비게이터 선택 단계 | 선택 항목 예시 |
| :--- | :--- |
| **1단계: 프로젝트 선택 (4 PJ)** | 프로젝트 A : 울산에 뿌리내리는 정주형 실전 인재 양성(Dynamic TALENT) |
| **2단계: 단위과제 선택 (12 UP)** | A1가 : 지역과 미래를 만드는 UC-HYPER 전문기술인재양성 |
| **3단계: 추진전략 선택 (S)** | S1 : 지역인력 수요분석 기반 UC-HYPER 실무인재 양성 |
| **4단계: 전략과제 선택 (T)** | T1 : UC-HYPER 기반 주문식 교육과정 및 혁신 교수법 개발 |

* **선택된 추진전략 (STRATEGY)**: `S1 지역인력 수요분석 기반 UC-HYPER 실무인재 양성`
* **선택된 전략과제 (STRATEGIC TASK)**: `T1 UC-HYPER 기반 주문식 교육과정 및 혁신 교수법 개발`
* **연계 프로그램 내역 (PROGRAMS)**: ➔ `UC-HYPER 교수법 개발(공학/비공학)` [프로그램 ID: `A1가-S1T1-1`]
