...
* **통합 [엑셀 다운로드] 메뉴**: 
  - 메인 화면 우측 상단 헤더 영역(로그아웃 버튼 바로 옆)에 **[엑셀 다운로드]** 메뉴가 배치되어 있습니다.
  - 이를 클릭하면 현재 선택된 연차의 **전체 예산 집행 총괄표, 프로젝트별 재원 배분 현황, KPI 성과 달성율 통계**를 단일 통합 엑셀 보고서 형태로 즉시 저장할 수 있습니다.
...
* **집행현황 [엑셀 다운로드] 버튼**:
  - 집행현황 테이블 우측 상단에 배치된 **[엑셀 다운로드]** 버튼을 통해 전체 단위과제의 예산 구성 및 프로그램 상태 집계 표 데이터를 즉시 다운로드하여 내부 행정용으로 편집할 수 있습니다.
...
* **P-D-C-A 단계별 기획-실적 수립 및 드롭다운 입력 형식**:
  - **12달 추진 일정 드롭다운 입력**: 대시보드 내 월별 일정 입력 창에서 12개월(3월 ~ 익년 2월) 전체의 사업 수행 단계를 각각 드롭다운 상자 형태로 배치하여, 실무진이 마우스 클릭만으로 P / D / C / A 값을 명확하고 일관되게 선택 입력할 수 있도록 통제합니다.
  - **Plan (P단계 기획)**: 국고, 지자체 시비, 외부사업비 등의 재원별 예산 배정액과 비목별 예산 한도를 기획합니다. (D1, D2, D3 단위과제 하위 프로그램은 시비 매칭 비율이 0%인 국비 100% 조건으로 통제됩니다.)
  - **Do (D단계 실적)**: 월별 실적 수행 단계와 실제 집행액 세부 재원을 기입합니다.
  - **Check / Act (C/A단계 점검/환류)**: 목표 대비 실적 횟수를 체크하고, 2분할 환류 조치 양식(우수사례 및 Deficiency 분석에 기반한 개선 행동 조치)을 상세 기입할 수 있습니다.
* **단위과제 연동 성과지표(KPI) 및 참여 대상자 드롭다운**:
  - **성과지표 연동 드롭다운**: 프로그램이 속한 단위과제(예: `A1가`)의 고유 목표지표 정보(예: `주문식 교육과정 참여 학생 수` 등)가 시스템적으로 실시간 바인딩되어 드롭다운 메뉴로 연동 노출됩니다. 연구원은 단순 기입 오류 없이 규정된 KPI 지표를 드롭다운 선택으로 매핑할 수 있습니다.
  - **참여 대상자 양식 규격화**: 참여 대상(학생, 교원, 기업체, 기타) 및 목표 수혜자 인원 필드를 현재 대시보드 엑셀 템플릿과 100% 정합성을 가지도록 통일성 있게 제공합니다.
...
* **반달(15일) 단위 간트 차트 (Gantt Chart) 조작**:
  - 마우스 드래그를 통해 일정 바의 좌/우측 끝을 늘리거나 줄여 일정을 수정합니다. 마우스를 놓는 즉시 날짜 범위 형식(`YYYY.MM.DD ~ YYYY.MM.DD`)으로 변환되어 DB에 실시간 저장됩니다.
* **프로그램별 P-D-C-A 전체 양식 엑셀 다운로드 (Excel Export)**:
  - 각 프로그램 상세 패널 우측 상단의 **[엑셀 다운로드]** 버튼을 통해 기획 단계(P)부터 최종 환류 단계(A)까지 적재된 데이터를 서식화된 단일 엑셀 문서로 즉시 출력할 수 있습니다.
  - **P단계 시트 정밀 가공**: 특히 기획(P) 단계의 재원별 예산 배정액(국비, 시비, 외부재원), 주요 비목별 한도, 연동된 성과지표 명칭, 규격화된 수혜 대상자 수치가 일목요연하게 엑셀에 로드되어, 정부 제출용 및 사업단 내부 보관용 문서로 바로 활용할 수 있도록 지원합니다.
