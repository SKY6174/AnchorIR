...
* **외부 만족도조사 결과 요약 텍스트 AI 분석 및 자동 필링**: 
  - 신설된 `기존 만족도조사 결과 입력 (AI)` 기능을 통해, 외부 플랫폼(구글폼, 네이버폼, 엑셀, 한글 문서 등)에서 실시한 만족도 조사 통계나 보고서 텍스트를 복사-붙여넣기하여 새 만족도 조사 데이터로 즉시 등록할 수 있습니다.
  - **ChatGPT(GPT-4o-mini)** 및 **Gemini(Gemini 1.5 Flash)** API 엔진을 선택하고 [AI 결과 분석 시작]을 누르면, 설문제목, 설문대상, 설문시기, 설문목적, 설문문항, 응답률(참여 응답 수), 종합 평균 만족도(100점 만점 환산), 주관식 의견 등의 핵심 정보를 AI가 문맥에서 자동으로 유추 및 추출하여 편집 폼의 빈 칸을 채워줍니다. (API 키가 없거나 통신 장애 시에도 스마트 로컬 정규식 파서가 차선책으로 자동 채우기를 지원합니다.)
  - 사용자는 AI가 채워준 항목을 직접 검토 및 수정한 뒤 [최종 등록]을 클릭하여, DB 테이블에 신규 설문 데이터와 종합 점수 분포에 부합하도록 정밀 조율된 개별 모의 응답자 행을 실시간으로 일괄 자동 생성하여 저장할 수 있습니다.
