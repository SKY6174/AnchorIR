    ...
    try {
      ...
      if (queryEmbedding && Array.isArray(queryEmbedding)) {
        ...
        if (dbChunks && dbChunks.length > 0) {
          ...
          const yearFiltered = dbChunks.filter(chunk => {
            const chunkYear = chunk.metadata?.year ? Number(chunk.metadata.year) : null;
            return !chunkYear || chunkYear === selectedYear;
          });
      ...
      console.warn("실시간 DB pgvector RAG 탐색 실패, 모의 데이터 폴백을 준비합니다:", e.message);
    }
...
3. 2차년도인지 1차년도인지 연차 구분이 질문에 언급되거나 문맥상 파악되면 해당 연도 기준 지침에 맞춤 대조하여 상세히 대답하십시오.`;
      ...
      setTimeout(() => {
        setMessages(prev => [...prev, {
          sender: "ai",
          text: ragResult.answer + `\n\n*(⚠️ 실시간 GPT 연결 실패로 모의 지식베이스 결과로 답변되었습니다. 원인: ${err.message})*`,
          sources: ragResult.sources
        }]);
        setLoading(false);
      }, 850);
