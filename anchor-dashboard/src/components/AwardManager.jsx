...
export default function AwardManager({
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        const storagePath = `${Date.now()}_${Math.random().toString(36).substring(2, 8)}${fileExt}`; // awards 전용 버킷 루트에 업로드
        ...
        setAwardFileData(publicUrl); // DB 및 상태값에 URL 저장
      } catch (err) {
  ...
  // 상장 파일 사본 열기 핸들러 (원격 URL 및 Base64 하이브리드 지원)
  ...
  const handleApplyBatchAwards = async () => {
    const successItems = batchAwardResults.filter(res => res.status === "success" && res.targetId);
    ...
    alert(`총 ${appliedCount}건의 상장 사본 파일이 스토리지 업로드 및 반영 완료되었습니다.`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
        <div>
          <h2 style={{ fontSize: "1.0rem", fontWeight: "800", color: "var(--text-primary)" }}>🏆 {selectedYear}차년도 상장 발급 내역</h2>
          <p style={{ fontSize: "0.72rem", color: "var(--text-secondary)" }}>공모전 및 경진대회 상장 발급 대장을 영속 보존합니다.</p>
        </div>
          ...
          <tbody>
            {sortedAwards.length === 0 ? (
              ...
              sortedAwards.map((award) => (
                <tr key={award.id} style={{ borderBottom: "1px solid var(--border-color-dark)", background: "transparent" }}>
                  <td style={{ padding: "0.6rem 0.8rem", fontWeight: "700" }}>{award.awardNo}</td>
                  <td style={{ padding: "0.6rem 0.8rem" }}>
                    <span style={{ color: "var(--text-secondary)", marginRight: "0.4rem" }}>[{award.recipientDept}]</span>
                    <strong style={{ color: "var(--text-primary)" }}>{award.recipientName}</strong>
                  </td>
