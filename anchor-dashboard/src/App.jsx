...
export default function App() {
  ...
  const [projects, setProjects] = useState(() => {
    // 1차년도 프로그램 ID 신규 규칙 적용 및 예산 정합성 보정을 위해 로컬스토리지 키 버전을 v14로 업그레이드합니다.
    const cached = localStorage.getItem("anchor_projects_data_v14");
    if (cached) {
      try {
  ...
  useEffect(() => {
    // 1차년도 프로그램 ID 신규 규칙 적용 및 예산 정합성 보정을 위해 로컬스토리지 키 버전을 v14로 업그레이드합니다.
    localStorage.setItem("anchor_projects_data_v14", JSON.stringify(projects));
  }, [projects]);
