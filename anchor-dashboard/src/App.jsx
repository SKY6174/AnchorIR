...
export default function App() {
  ...
  useEffect(() => {
    const handleGlobalError = (event) => {
      ...
      localStorage.removeItem("anchor_projects_data_v24");
  ...
  const [projects, setProjects] = useState(() => {
    // 2차년도 세부 프로그램 ID 및 예산 데이터를 PDF 실증 데이터로 정밀 동기화하기 위해 로컬스토리지 버전을 v24로 업그레이드합니다.
    const cached = localStorage.getItem("anchor_projects_data_v24");
    if (cached) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("anchor_projects_data_v24", JSON.stringify(projects));
    } catch (e) {
      ...
      if (isQuotaError) {
        ...
        try {
          Object.keys(localStorage).forEach((key) => {
            if (key.startsWith("anchor_projects_data_") && key !== "anchor_projects_data_v24") {
              localStorage.removeItem(key);
            }
          });
          localStorage.setItem("anchor_projects_data_v24", JSON.stringify(projects));
