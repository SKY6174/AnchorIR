...
export default function App() {
  ...
  useEffect(() => {
    const handleGlobalError = (event) => {
      ...
      localStorage.removeItem("anchor_projects_data_v21");
  ...
  const [projects, setProjects] = useState(() => {
    // D3 1차년도 실제 예산 및 집행 데이터 롤업 정합성 연동을 위해 로컬스토리지 키 버전을 v21로 업그레이드합니다.
    const cached = localStorage.getItem("anchor_projects_data_v21");
    if (cached) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("anchor_projects_data_v21", JSON.stringify(projects));
    } catch (e) {
      ...
      if (isQuotaError) {
        ...
        try {
          Object.keys(localStorage).forEach((key) => {
            if (key.startsWith("anchor_projects_data_") && key !== "anchor_projects_data_v21") {
              localStorage.removeItem(key);
            }
          });
          localStorage.setItem("anchor_projects_data_v21", JSON.stringify(projects));
