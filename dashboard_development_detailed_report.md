...
## 9. 👥 신규 구성원 온보딩 및 로그인 계정 추가 영향도 가이드

사업단에 신규 인력이 합류하여 대시보드 로그인 권한과 조직도/주소록 명단을 추가해야 하는 경우, 시스템 일관성을 위해 다음의 데이터베이스(DB) 및 소스코드(FE) 내역을 현행화해야 합니다.

### 1) 데이터베이스(DB) 테이블 영향 범위

1. **`rise_members` 테이블 (주소록 명단)**
   - **설명**: 대시보드의 '구성원 관리' 판넬 및 조직도 뷰에서 활용되는 마스터 명단 테이블입니다.
   - **컬럼**: `id`(예: `m-44`), `name`(성함), `role`(직책), `grade`(직급), `dept`(부서), `phoneOffice`, `phoneMobile`, `email`, `room`, `startDate`, `status`
   - **영향**: 이 테이블에 데이터가 등록되어 있어야만 대시보드 내의 현황 및 권한 조회 화면에서 정상 노출됩니다.
2. **`rise_users` 테이블 (로그인 테이블)**
   - **설명**: 프런트엔드의 이메일/패스워드 로그인 시 직접적인 자격증명(Credentials)을 인증하는 테이블입니다.
   - **컬럼**: `id`(이메일, PK), `pw`(비밀번호의 SHA-256 해시값), `name`(성함 및 직책), `role_key`(역할 권한 구분자 예: `RESEARCHER`, `TEAM_LEADER` 등), `approved`(기본값 true)
3. **`auth.users` 테이블 (Supabase Auth)**
   - **설명**: 실제 세션 발급 및 JWT 토큰 보안을 담당하는 Supabase 내장 인증 테이블입니다.
   - **영향**: `rise_users`와 `auth.users`가 상호 연동되어 있어야 로그인이 정상 작동합니다. 마이그레이션 함수 `public.reset_all_member_passwords_v3()`를 기동하여 `rise_members` 데이터를 기반으로 두 테이블의 데이터를 일괄 생성 및 동기화할 수 있습니다.

### 2) 소스코드 및 정적 설정 파일 영향 범위

1. **`anchor-dashboard/src/App.jsx` 내 `initialList` 데이터**
   - **위치**: 대시보드 컴포넌트 내부의 초기 구성원 데이터셋 정의부 (Line 305 부근)
   - **역할**: 로컬 스토리지에 캐시된 탭이나 명단 정보가 없거나, DB 서버 네트워크 로드 지연 시 화이트스크린을 방어하는 하이브리드 폴백 데이터셋입니다. 신규 인력 추가 시 이 배열에도 정보를 추가해야 합니다.
2. **Supabase 마이그레이션 파일 (`supabase/migrations/`)**
   - **위치**: `004_insert_member_users.sql` 및 `006_create_members_table.sql`
   - **역할**: 추후 서버 환경을 새로 구축(Re-build)하거나 로컬 개발 데이터베이스를 초기화(Seed)할 때, 신규 데이터 누락을 방지하고 코드의 형상과 DB 데이터 형상을 완벽히 일치시키기 위한 아카이브 저장소입니다. 해당 SQL 파일의 `INSERT` 구문에 신규 데이터를 업데이트합니다.
