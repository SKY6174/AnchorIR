...
- **센터장 및 팀장 공식 역할키 매핑**:
  - 홍광표 (늘봄누리센터장) ➔ `CENTER_NURI`
  - 김기범 (ICC센터장) ➔ `CENTER_ICC`
  - 현용환 (RCC센터장) ➔ `CENTER_RCC`
  - 이동은 (ECC센터장) ➔ `CENTER_ECC`
  - 홍진숙 (신산업특화센터장) ➔ `CENTER_SPECIAL`
  - 팀장교수/팀장 (이한도, 고형석, 양승호, 이정준, 장광일, 김민경, 박성혁, 이상현, 김산, 한미라) ➔ `TEAM_LEADER`
