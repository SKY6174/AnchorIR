...
* **ID 형식 규칙 (단위과제 및 프로그램 번호)**:
  ...
  - **1차년도(2025년) 프로그램 ID**: `{단위과제ID}-S{추진전략번호}T{추진과제번호}-{추진과제내 프로그램순서}` 형식을 따릅니다. (예: `A1-S1T2-3` - 단위과제 A1, 전략 S1, 추진과제 T2의 3번째 프로그램)
  - **2차년도(2026년) 이상 프로그램 ID**: `[단위과제ID]-[2자리패딩번호]` 형식으로 정의합니다. (예: `A1가-01`, `A1가-12`, `A2-01`, `A2-10`)
...
1.  **PDCA 단계별 표준 프로세스 및 검증**:
    *   **P (Plan) 단계**:
        ...
        *   *월별 추진 일정 (PDCA) 레이아웃*: 12개월의 기획 일정 선택 필드는 화면 가로 공간의 효율적 활용 및 가독성 유지를 위해 **2줄 Grid (1행당 6개월씩, 총 6열 2행 구조)**로 레이아웃을 분할하여 배치합니다.
        ...
        *   *타임라인 대각선 그라데이션 시각화*: 하나의 달에 2가지 이상의 다중 단계(예: `P/D`, `D/C` 등)가 중첩 기획된 프로그램의 경우, "프로그램 진행" 탭의 모니터링 타임라인 상에서 해당 셀의 배경색을 CSS `linear-gradient(135deg, 색상1 50%, 색상2 50%)` 를 이용해 **대각선으로 대칭 반분되는 색상**으로 정교하게 렌더링하고, 글자 크기를 살짝 좁혀 시각적 대비를 제공합니다.
    *   **D (Do) 단계**:
...
4.  **예산항목 관리**: 각 단위과제별 세부 비목의 본사업비와 이월예산 배정액 한도를 통제하고 상세 실적을 확인하는 재정 관리 공간입니다. 좌측 단위과제 목록은 **공통경비를 최상단에 배치**하고, **담당부서별(ECC센터 -> ICC센터 -> RCC센터 -> AID-X지원센터 -> 울산늘봄누리센터 -> 신산업특화센터 -> 사업운영팀)로 그룹핑하여 시각적 정렬**을 보장합니다. 단, 1차년도의 경우에는 신산업특화센터를 아예 정렬 및 노출 목록에서 제외하여 연차별 정합성을 수립합니다.
