...
- **가중치 적합도 매칭 알고리즘**: 업로드된 파일명에서 언더바(`_`) 패턴을 기반으로 협약기관 명칭을 정밀 추출하고, 괄호 `(...)` 패턴을 통해 주체 성명(교수명 등)을 정밀 분리하여 대조합니다. 가중치 적합도 점수는 협약기관 일치 50점, 주체 성명 일치 40점, 연도 일치 30점, 구체적 체결날짜 일치 +10점 보너스로 가산 합산합니다.
