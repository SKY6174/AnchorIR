...
- **가중치 적합도 매칭 알고리즘**: 업로드된 파일명에서 언더바(`_`) 패턴을 기반으로 협약기관 명칭을 정밀 추출하되, 반각/전각 회사 수식어구(예: `(주)`, `(유)`, `주식회사`, `유한회사` 및 유니코드 `㈜`, `㈔` 등)는 완전히 제외하고 일반 괄호까지 소거한 핵심 브랜드명만을 비교 대조합니다 (이때 주 뒤에 임의의 공백이 섞여 있는 `( 주 )` 와 같은 불안정한 패턴까지 공백 유연 매칭 정규식 `\s*` 으로 완벽하게 파악하여 탈락시킵니다. 또한, Vite 등의 프론트엔드 빌더가 코드를 압축(Minify)할 때 정규식 리터럴 `/.../` 내의 백슬래시 이스케이프 `\`를 임의로 탈락시켜 `(주)`의 괄호가 매칭되지 않고 괄호 기호만 둥둥 뜨는 런타임 결함(예: `주레디메이드` 로 오파싱)을 완전히 차단하기 위해, 법인명 제거 정규식은 반드시 RegExp 생성자인 `new RegExp(..., "g")`에 이중 이스케이프 백슬래시 `\\`를 결합하여 소스코드를 엄밀히 구조화해야 합니다). 또한, 대시보드 데이터 측의 협약기관(`item.organizations`) 데이터 타입이 단일 문자열, 단순 텍스트 배열, 혹은 내포된 객체 배열 등 다양하게 유입될 수 있으므로, 타입 검증을 거쳐 문자열 명칭만 안전하게 정화한 뒤 대조해야 합니다. 가중치 적합도 점수는 파일명 추출 기관명이 대시보드의 협약기관 텍스트에 포함(Inclusion)되면 50점, 파일명 추출 성명이 대시보드의 협약주체 텍스트에 포함(Inclusion)되면 40점, 연도 일치 시 30점, 구체적 체결날짜 일치 시 +10점 보너스로 가산 합산합니다.
