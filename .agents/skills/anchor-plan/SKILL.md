...
- **지원 규모**:
 ...
 - **재원 구분 및 표기 가이드라인 (연차별 회계 규칙)**:
  ...
  - **2차년도 D1 및 D3 단위과제 예산 보정 규칙**: D1, D3 단위과제 및 소속 모든 세부 프로그램들의 2차년도 본예산 편성 시, **국비(국고) 비율 100%**로 고정 편성하고 **시비(지자체 매칭) 예산은 전면 제외(0원)**합니다. 이월예산이나 집행 실적 롤업 시에도 이 원칙이 적용되도록 처리합니다.
