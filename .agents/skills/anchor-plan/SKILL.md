...
- **가중치 적합도 매칭 알고리즘**: 업로드된 파일명에서 정규식으로 추출한 날짜 및 문자열 정화 처리를 한 기관명, 발급번호, 담당교수 성명을 대시보드 내의 원본 데이터와 교차 대조하여 적합도 점수(기관명 50점, 날짜 30점, 담당자 20점 등)를 매깁니다.
- **신뢰성 보장 한계선**: 무작위 연결 오류를 방지하기 위해 가중치 총점이 최소 50점 이상인 경우에만 자동 매칭 성공으로 간주하고, 결과를 리포트 테이블로 띄워 사용자의 일괄 최종 승인을 거쳐 일괄 `Update` 처리해야 합니다.