...
- **내부 스크롤 메커니즘**: 모달의 헤더와 제목 및 닫기 버튼은 상단에 고정하고(`flexShrink: 0`), 하단 경계선과 저장 버튼이 화면 아래 바깥으로 잘리지 않도록 겉 레이어를 화면 정중앙에 플렉스 정렬하여 viewport 내로 강제 수용시킵니다. 실제 입력 칸을 품는 양식 영역(`<form>`)에만 `flex: 1` 및 `overflowY: "auto"` 속성을 부여해 내부 스크롤이 작동하도록 레이아웃을 엄격하게 설계해야 합니다.