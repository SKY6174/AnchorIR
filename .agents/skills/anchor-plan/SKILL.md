...
1.  **IR 대시보드**: 전체 사업비의 연도별 요약, 본사업비/이월사업비 집행률, 재원 배분율 및 누적 집행 현황 그래프 등을 시각화하여 대외 보고를 지원합니다.
    *   **총사업비 요약 카드 구성**:
        ...
        4.  *상세 지표 카드형 배지 구조*: 상세 리스트의 가독성 향상과 직관적 구분을 위해, 각 사업비 지표와 금액 수치는 **텍스트 중앙 정렬** 처리하고 **둥근 모서리 미니 카드 사각형(`border-radius: 0.375rem`, 연한 경계선)**으로 둘러싸 배치합니다. 채우기 색상은 재원별 성격(앵커 본사업은 옅은 블루, 이월은 옅은 퍼플, 신산업 본사업은 옅은 그린, 신산업 이월은 옅은 골드)에 맞춘 고유 파스텔 톤 배경색으로 칠하여 시각적으로 확실히 분리합니다.
        5.  *그리드 카드 배치 분기 구조*: 상세 정보가 많아 넓은 공간이 필요한 **첫 번째 카드(총 예산)에 2배 크기 너비(`2fr`)를 강제 부여**합니다. 우측에 배치되는 두 집행 카드(본사업비 집행, 이월사업비 집행)는 하나의 세로 정렬 컨테이너(`display: flex; flex-direction: column`)로 묶어 **아래위로 정렬**함으로써 좌측 총 예산 카드의 높이와 대칭을 이루도록 구성합니다. 1차년도 뷰일 때는 우측 1fr 공간에 본사업비 집행(위)과 KPI 달성도(아래)가 아래위로 포개어 배치되고, 2차년도 이상일 때는 본사업비 집행과 이월사업비 집행이 가운데 열에 아래위로 포개어지고 KPI 달성도가 우측 1fr 열에 배치되는 대칭 그리드로 설계합니다.
    *   **차트 시각화 및 UX 규격**:
