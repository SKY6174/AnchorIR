...
1.  **IR 대시보드**: 전체 사업비의 연도별 요약, 본사업비/이월사업비 집행률, 재원 배분율 및 누적 집행 현황 그래프 등을 시각화하여 대외 보고를 지원합니다.
    ...
    *   **차트 시각화 및 UX 규격**:
        1.  *도넛 차트 (재원 배분 구조)*: 각 조각(Sector) 옆에 연한 지시선(`labelLine`, 투명도 25%)을 연결하고, `프로젝트 명 (비율%)` 형태로 범례 라벨을 직접 표출하여 차트 정보의 완결성을 높입니다. 텍스트가 잘리지 않도록 PieChart 마진(Margin)을 좌우로 충분히 확보합니다.
