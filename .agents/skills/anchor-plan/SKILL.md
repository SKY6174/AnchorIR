...
- **내부 스크롤 및 중앙 정렬 메커니즘**: 모달이 사용자가 바라보는 화면(Viewport)의 수직/수평 정중앙에 완벽하게 안착하도록 겉 레이어(overlay wrapper)에 `display: "flex"`, `justifyContent: "center"`, `alignItems: "center"`, `overflowY: "auto"`, `padding: "2rem 1rem"`을 부여하고, 실제 모달 몸체(container)에는 `margin: "auto"`를 명시하여 브라우저 스크롤 시 상단 잘림이나 쏠림 현상을 원천 방지합니다. 모달 컨테이너 자체는 `maxHeight: "85vh"`를 가지며 헤더는 상단 고정(`flexShrink: 0`)하고, 실제 입력 칸을 품는 양식 영역(`<form>`)에만 `flex: 1` 및 `overflowY: "auto"` 속성을 부여해 내용물만 내부 스크롤이 작동하도록 설계하여, 하단 버튼이 뷰포트 아래로 잘려나가지 않고 항상 화면 중앙 스케일 내에 존재하도록 제어합니다.
...
- **가중치 적합도 매칭 알고리즘**: 업로드된 파일명에서 정규식 및 단순 문자열 대조로 추출한 연도, 날짜, 기관명, 발급번호, 담당자 성명을 대시보드 내의 원본 데이터와 교차 대조하여 적합도 점수(기관명 일치 40점, 성명 일치 30점, 연도 일치 20점, 정밀 날짜 일치 10점 등)를 매깁니다.
- **신뢰성 보장 한계선**: 매칭의 성공률을 극대화하기 위해 매칭 신뢰성 보장 최소 컷오프 기준 장벽을 20점 이상(즉, 기관명이나 성명, 연도 중 최소 하나만 확실히 일치해도 매칭 허용)으로 완화하여 탐색률을 비약적으로 높이며, 결과를 리포트 테이블로 띄워 사용자의 일괄 최종 검토를 거쳐 `Update` 처리되도록 구성합니다.