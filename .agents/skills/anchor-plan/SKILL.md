...
- **지원 규모**:
 ...
 - **재원 구분 및 표기 가이드라인 (연차별 회계 규칙)**:
  - 1차년도 사업비는 본사업비로만 구성되며 이월예산이 아예 존재하지 않습니다. 또한, 1차년도(2025년)에는 공통경비(운영팀 공통운영경비) 예산이 별도로 존재하지 않으므로, 대시보드 및 재원 통계 연산 시 공통경비 프로젝트는 1차년도 데이터 집계 및 시각화 차트에서 완전 제외 처리합니다.
...
* **ID 형식 규칙 (단위과제 및 프로그램 번호)**:
  ...
  - 프로그램 ID는 `[단위과제ID]-[2자리패딩번호]` 형식으로 정의합니다. (예: `A1가-01`, `A1가-12`, `A2-01`, `A2-10`)
  - 신규 프로그램 생성 시에도 이 자릿수 패딩 룰을 반드시 준수하여 고유 번호를 부여해야 합니다.
...
1.  **PDCA 단계별 표준 프로세스 및 검증**:
    *   **P (Plan) 단계**:
        *   *등록 필수 항목*: 추진 일정 (Timeline), 참여 대상 (Target), 연계 부서 (Coop Dept) 기획 정보, 계획 횟수 (회), 및 재원별 예산 배정액.
        *   *완료 조건*: 배정 예산이 0원을 초과해야 하고, 추진 일정, 참여 대상, 연계 부서 텍스트가 모두 기입되어야 완료 처리 가능.
    *   **D (Do) 단계**:
        *   *등록 필수 항목*: 세부 재원별 실제 집행 실적 입력 (국고 집행액, 시비 집행액, 외부사업비 집행액) 및 수행 실적 (수행 횟수(회), 최종 이수인원(명), 계획대비 달성률(%)).
        *   *레이아웃 구조*: 집행 등록 폼은 가독성을 위해 **두 줄의 Grid**로 분할 배치합니다. 첫 번째 줄에는 예산 집행액(국고, 시비, 외부)을 배치하고, 두 번째 줄에는 실적 지표(수행 횟수, 이수인원, 달성률)를 한 줄에 병렬 배치합니다.
        *   *완료 조건*: 누적 총 집행액이 0원 초과이고, 이수인원 및 수행 횟수가 0보다 커야 완료 처리 가능.
    *   **C (Check) 단계**:
...
1.  **IR 대시보드**: 전체 사업비의 연도별 요약, 본사업비/이월사업비 집행률, 재원 배분율 및 누적 집행 현황 그래프 등을 시각화하여 대외 보고를 지원합니다.
    *   **총사업비 요약 카드 구성**: 'ANCHOR 2차년도 총 예산' 카드의 요약 정보는 총 3줄의 좌우 대칭 레이아웃으로 구분하여 표출합니다.
        1.  *1행*: `앵커(본사업)` 및 `앵커(이월사업)`
        2.  *2행*: `신산업(본사업)` 및 `신산업(이월사업)` (단위과제 ID `A1나` 전용 추출 데이터)
        3.  *3행*: `2차년도(본사업)` (앵커+신산업 본사업 합산) 및 `1차년도(이월사업)` (앵커+신산업 이월비 합산)
    *   **차트 시각화 및 UX 규격**:
        1.  *도넛 차트 (재원 배분 구조)*: 각 조각(Sector) 옆에 연한 지시선(`labelLine`, 투명도 25%)을 연결하고, `프로젝트 명 (비율%)` 형태로 범례 라벨을 직접 표출하여 차트 정보의 완결성을 높입니다. 텍스트가 잘리지 않도록 PieChart 마진(Margin)을 좌우로 충분히 확보하며, 정보의 중복 노출을 피하기 위해 하단의 별도 범례(Legend) 목록 영역은 삭제하여 깔끔한 공간 균형을 연출합니다.
        2.  *막대 그래프 (프로젝트별 재원/집행 현황)*: 마우스 호버 시 노출되는 활성 가이드 영역(Cursor)의 배경색 대비가 다크 모드 화면에서 시각 피로를 주지 않도록 투명 채우기(`rgba(255, 255, 255, 0.03)`) 처리를 하여 부드럽게 감싸줍니다. 툴팁 팝업 내부 글씨는 `11px`로 압축하여 컴팩트한 시각적 균형을 유지합니다.
...
| D2. U-MedTown 지역 완결형 의료체계 구축 | 11.0 | 미지정 |
| 최승혜 | 40.0 | 최승혜 |
...
| D3. 지역과 문화가 함께하는 매력있는 꿀잼도시 조성 | 390.0 | 미지정 |
