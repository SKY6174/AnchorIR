...
- **날짜 기반 연차 자동 도출**: 데이터 저장 시(협약서, 이수증, 상장) 입력한 일자를 기준으로 사업 연차(1~5차년도)를 실시간으로 판정합니다. (RISE 사업 연차 기준: 3월 1일 ~ 익년 2월 말일이 해당 연차 범위)
...
- **모달창 세로 규격 확보**: 입력 요소가 많고 동적으로 변화하는 협약서 등록/수정 모달창 등 활성화 팝업의 경우, 저해상도 화면이나 특정 아이프레임 샌드박스 내부에서도 입력 폼 전체가 축소되어 답답해 보이지 않도록 기본 높이를 뷰포트의 85%(`height: "85vh"`)로 충분하고 시원하게 선언하여 넓은 입력 영역을 보장합니다.
- **내부 스크롤 및 중앙 정렬 메커니즘**: 모달이 사용자가 바라보는 화면(Viewport)의 수직/수평 정중앙에 단정하게 표시되도록 겉 레이어(overlay wrapper)에 `display: "grid"`, `placeItems: "center"`, `overflowY: "auto"`, `padding: "2rem 1rem"`을 부여합니다. 모달 컨테이너 자체는 85vh의 세로 높이를 가지며 헤더는 상단 고정(`flexShrink: 0`)하고, 실제 입력 칸을 품는 양식 영역(`<form>`)에만 `flex: 1` 및 `overflowY: "auto"` 속성을 부여해 내용물만 내부 스크롤이 작동하도록 설계하여, 하단 버튼이 뷰포트 아래로 잘려나가지 않고 항상 화면 중앙 스케일 내에 존재하도록 제어합니다.