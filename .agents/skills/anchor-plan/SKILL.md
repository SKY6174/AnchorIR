...
11. **주소록 기반 자동 계정 매핑 및 퇴직자 차단**: 별도의 수동 회원가입 절차 없이, 구성원 주소록(재직중)에 이메일과 정보가 등록되어 있다면 자동으로 계정이 활성화됩니다. 초기 로그인 비밀번호는 본인의 휴대전화 번호 뒷자리 4자리이며, 로그인 시 직책명에 따라 상응하는 역할 권한(단장, 본부장, 센터장, 운영팀장, 연구원 등)이 동적으로 부여됩니다. 주소록에서 퇴직으로 상태가 변경되면 시스템 진입은 실시간으로 원천 차단됩니다.
