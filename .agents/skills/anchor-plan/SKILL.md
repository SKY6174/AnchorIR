---
name: anchor-plan
description: "울산과학대학교 라이즈(앵커) 사업 기획 체계를 '추진전략 - 전략과제 - 프로그램 (세부 행동방식: 액션플랜)' 구조로 정의하고, 사업계획 및 예산 배분을 지원하는 스킬입니다."
---

# ⚓ 울산과학대학교 라이즈(앵커) 사업단 기획 체계 가이드 (anchor-plan)

이 스킬은 울산과학대학교 라이즈(앵커) 사업단이 프로젝트 관리를 위해 **추진전략 - 전략과제 - 프로그램**의 3단계 핵심 기획 체계를 구축하고, 각 프로그램을 실제로 수행하기 위한 세부 실행 수단으로서 **표(Table) 형식의 액션플랜**을 구성하도록 돕는 가이드 및 에이전트 행동 지침입니다.
...
- **지원 규모**:
 ...
 - **재원 구분 및 표기 가이드라인 (연차별 회계 규칙)**:
  ...
  - **1차년도 공통경비 처리 예외 규칙**: 1차년도 사업의 경우 2차년도와 달리 프로젝트 수준에서의 전사 공통경비 편성이 불가합니다. 따라서, 1차년도에 한해 모든 단위과제(A1~D4) 하부에 '공통경비' 전략과제(T)와 '공통경비' 프로그램(Pg)을 반드시 추가하여 운영해야 합니다. 해당 '공통경비' 프로그램의 세부 액션플랜(Action Plan) 표에는 다른 세부 프로그램들에 반영되지 않은 공통 성격의 인건비, 간접비, 그 밖의 사업운영경비, 성과활용확산 지원비 등의 집행 계획을 개별 행으로 명시적으로 기재하여 관리해야 합니다.
  - **2차년도 D1, D2, D3 단위과제 예산 보정 규칙**: D1, D2, D3 단위과제 및 소속 모든 세부 프로그램들의 2차년도 본예산 편성 시, **국비(국고) 비율 100%**로 고정 편성하고 **시비(지자체 매칭) 예산은 전면 제외(0원)**합니다. 이월예산이나 집행 실적 롤업 시에도 이 원칙이 적용되도록 처리합니다.
...
graph TD
    ...
    Task["전략과제 (Strategic Task)<br>전략 달성을 위한 핵심 분야 (기존 S 하위 통합)"]
    Program["프로그램 (Program)<br>예산 및 KPI 관리의 핵심 단위 (전략과제별 1~3개)"]
...
### 2) 전략과제 (Strategic Task)
* **정의**: 최상위 추진전략을 달성하기 위한 구체적인 중점 과제 및 추진 분야입니다. 대시보드 내에서는 선택된 추진전략에 연계하여 `S1과 연계한 전략과제 (Strategic Tasks)` 와 같이 한글 조사 결합 법칙(받침이 있는 경우 '과', 없는 경우 '와')에 따라 맞춤법에 맞는 연쇄 렌더링을 정의합니다.

### 3) 프로그램 (Program)
* **정의**: 전략과제를 실현하기 위한 구체적인 실무 사업 단위이자 **예산과 성과 지표(KPI)가 배분되는 핵심 기획 관리 단위**입니다.
...
* **5단계 기획 위계 체계**:
  ...
  - **단위과제 (Unit Project; UP)** : 프로젝트 목표 달성을 위한 12대 단위과제 (1차년도: A1~D4, 2차년도: A1가~D3)
    ...
    * `D3` : 에코 컬처로 만드는 꿀잼도시 울산
  - **추진전략 (Strategy; S)**, **전략과제 (Strategic Task; T)**, **프로그램 (Program; PG)**
...
* **ID 형식 및 작명 규칙 (ID Rule)**:
  ...
  - **전략과제(T) 번호 부여**: 한 단위과제 내의 모든 전략과제는 소속 추진전략(S)과 관계없이 1부터 겹침 없이 순서대로 순차적인 번호(`T1`, `T2`, `T3`...)를 부여받습니다.
  - **프로그램 ID 생성 공식**: `[단위과제번호]-[추진전략번호 + 전략과제번호]-[해당 전략과제 내에서의 프로그램 순번]` 형식을 따릅니다.
    * 예시: `A1가-S1T1-1` ➔ 단위과제 `A1가`, 추진전략 `S1`, 전략과제 `T1`에 매핑된 **해당 전략과제(T1) 내 1번째 프로그램**을 의미함.
...
## 2. 전략과제 (Strategic Task) 1: [과제명] (S의 하위 과제로 1~2개 수립)
> [과제 개요 서술]

---

## 3. 프로그램 (Program) 1-1: [프로그램명] (전략과제별 1~3개 프로그램으로 구체화)
...
17. **회의록 소속 연구원 참여 상태 및 날짜 기반 필터링**: 회의록 등록/수정 모달에서 소속 연구원을 칩 목록으로 보여줄 때는, 해당 회의록의 회의 일자(`meetingDate`)를 기준으로 각 연구원의 시작일(`startDate`/`hireDate`)과 종료일(`endDate`) 범위 내에 들어있고 상태가 `'참여중'`인 활성 구성원만 포함시키도록 합니다. 예를 들어, 2026년 7월 1일이 회의 일자라면 이 일자에 미참여 상태이거나 재직 기간 외인 인원(예: 유리나, 이준우 등)은 목록에서 자동으로 배제되어야 합니다.
18. **보도자료(Press Release) 날짜 기반 연차 동적 분류 및 위치 수납 규칙**:
    - 사용자가 보도자료를 신규 등록, 수정, 혹은 패치 조회할 때, 화면에 활성화되어 있는 현재 탭 연차(`selectedYear`)나 데이터베이스의 단순 `year` 칼럼 매핑에 의존해서 고정 저장/표시하지 않습니다.
    - 보도자료의 실제 기사 발행일자(`broadcastDate` / `broadcast_date`)가 속하는 공식 회계연도 사업기간(매년 3월 1일 ~ 이듬해 2월 28일/29일)을 기준으로 연차를 실시간 계산하여 정확한 해당 연차 탭 목록에만 분류 수납되어 노출되도록 제어해야 합니다.
    - 예시: 현재 화면의 상단 탭 필터가 `2차년도(현)`에 선택되어 있는 과도기 상태에서, 새로 입력한 뉴스 기사의 진짜 보도일자가 `12/25/2025` (2025-12-25)라면, 해당 기사는 1차년도 사업범위(2025.03 ~ 2026.02)에 속하므로 등록 버튼을 누르는 즉시 화면상 1차년도 위치에 완벽하게 자동 소속되어 정돈되도록 통제합니다.
...
## 13. 1차년도 단위과제별 예산 및 전략, 전략과제 요약 (A1, A2, B1, B2, B3)

1차년도(2025년) 라이즈(RISE) 단위과제 고도화 및 정비 결과로 도출된 예산 배정 계획과 최상위 추진전략(S) 및 전략과제(T) 매핑 정보는 다음과 같습니다.
...
- **가중치 적합도 매칭 알고리즘**: 업로드된 파일명에서 언더바(`_`) 패턴을 기반으로 협약기관 명칭을 정밀 추출하되, 빌드 시 백슬래시 유실 장애를 막기 위해 **공백을 선제적으로 완전히 지운 뒤(`replace(/\s/g, "")`)에 단순한 `(주)` 등의 리터럴 RegExp 생성자를 실행**하여 회사 수식어구(예: `(주)`, `(유)`, `주식회사`, `유한회사` 및 유니코드 `㈜`, `㈔` 등)를 완벽하게 배제합니다. 매칭 합격은 파일명 추출 기관명이 대시보드의 협약기관 텍스트와 양방향 포함 관계에 있고, 파일명 추출 성명이 대시보드의 협약주체(학과+성명) 텍스트에 포함(Includes)될 때 성공(100점) 처리됩니다.
- **신뢰성 보장 한계선**: 매칭의 신뢰성을 보장하기 위해 두 조건(기관명 포함 && 성명 포함)이 동시에 충족된 100점 대상만 성공 매핑으로 통과(OK) 처리하며, 하나라도 미달하면 0점 실패로 처리합니다.
- **매칭 세부 판정식 리포팅 시각화**: 사용자가 매칭의 타당성을 완벽하게 감사(Audit)할 수 있도록, 일괄 매핑 결과 리포트 모달창의 표(Table) 상에 파일명 외에도 정규식으로 자동 추출된 핵심 정보(추출기관, 추출성명)와 가중치 합산 근거 점수 내역(예: `기관명:일치 + 성명:일치`)을 직관적인 뱃지 형태로 함께 노출하여 가시성을 보증해야 합니다.