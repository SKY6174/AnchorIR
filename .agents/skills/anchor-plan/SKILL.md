...
11. **주소록 기반 가입 및 퇴직자 로그인 제한**: 회원가입 시 반드시 구성원 주소록에 등록된 이메일과 이름만 가입 신청이 가능하도록 제한하며, 초기 비밀번호는 주소록상 휴대전화 번호 뒷자리 4자리로 자동 매핑합니다. 또한 계정이 존재하고 승인 상태일지라도, 주소록에서 퇴직 상태로 전환된 구성원의 경우 시스템 로그인을 원천 차단하여 정보 노출을 완벽하게 통제합니다.
