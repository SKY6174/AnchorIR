...
3.  **재원별 예산 엑셀 양식 연동 및 비목 롤업**:
    *   *엑셀 레이아웃 구성*: 예산 엑셀 양식은 단위과제ID, 프로그램ID, 프로그램명, 세부 재원별 배정액(국고, 시비, 외부 본예산 및 이월예산)과 10대 표준 비목이 우측으로 나열되는 구조를 가집니다.
    *   *비목 롤업 적용*: 엑셀 업로드 시 10대 비목 중 본예산이나 이월비 중 하나라도 금액이 0보다 큰 비목만 필터링하여 프로그램의 비목 정보(`budget_categories`)에 롤업합니다.
