...
- **가중치 적합도 매칭 알고리즘**: 업로드된 파일명에서 연도(연차), 기관명, 성명(담당자/수혜자) 키워드를 추출 및 대조하여 가중치 적합도 점수(협약기관 일치 40점, 성명 일치 30점, 연도 일치 30점, 정밀 날짜 일치 +10점 보너스 등)를 합산 채점합니다.
- **신뢰성 보장 한계선**: 1개 요소 단독 매칭으로 인한 오매칭 오류를 최소화하고, 다차원 교집합(예: 연도+기관명, 기관명+성명, 연도+성명 등 최소 두 개 이상의 핵심 키워드 중복 결합)을 확인하기 위해 매칭 신뢰성 최소 컷오프 장벽을 50점 이상으로 설정하며, 최종 일치 내역 리포트를 사용자에게 피드백 제공 후 일괄 `Update` 되도록 제어합니다.