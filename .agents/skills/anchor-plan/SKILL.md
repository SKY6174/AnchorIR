...
- **지원 규모**:
 ...
 - **재원 구분 및 표기 가이드라인**:
  - 모든 예산은 **"2026년 본사업비"**와 **"2025년 이월사업비"**로 엄격히 구분하여 예산 및 집행 현황을 병행 추적하고 기록 관리합니다.
...
1.  **PDCA 단계별 표준 프로세스 및 검증**:
    ...
    *   **D (Do) 단계**:
        *   *등록 필수 항목*: 세부 재원별 실제 집행 실적 입력 (국고 집행액, 시비 집행액, 외부사업비 집행액).
        *   *완료 조건*: 누적 총 집행액이 0원 초과이어야 완료 처리 가능.
    *   **C (Check) 단계**:
        *   *등록 필수 항목*: 프로그램 최종 이수인원(명) 및 수요자 만족도(%) 점수 기재 (**실집행액 입력/검증은 C 단계에서 제외**).
        *   *완료 조건*: 이수인원이 0명 초과, 만족도가 0% 초과이어야 완료 처리 가능.
    *   **A (Act) 단계**:
        *   *등록 필수 항목*: 프로그램 성과 성격에 따른 **2분할 환류 방안** 기재.
            1.  *우수 프로그램*일 때 -> `우수한 점` 및 `발전방안` 필수 기재.
            2.  *미흡 프로그램*일 때 -> `미비점` 및 `개선사항` 필수 기재.
