...
-- 1. rise_users 테이블 행 수준 보안(RLS) 활성화 및 기존 익명 SELECT 정책 제거
ALTER TABLE rise_users ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow select on rise_users for auth mapping" ON rise_users;
DROP POLICY IF EXISTS "Allow select for login verification" ON rise_users;
DROP POLICY IF EXISTS "Allow authenticated changes on rise_users" ON rise_users;
DROP POLICY IF EXISTS "Allow insert/update for auth users on rise_users" ON rise_users;

-- 비로그인 익명(anon) 사용자에게는 rise_users 테이블 조회 권한을 완벽 차단
-- 로그인에 성공한 인증 회원(authenticated)만 자신의 데이터 매핑 및 비밀번호 변경 조회를 수행하도록 한정
CREATE POLICY "Allow select on rise_users for authenticated only" ON rise_users
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow all changes on rise_users for authenticated only" ON rise_users
...
CREATE POLICY "Allow select for all on unified_certificates" ON unified_certificates
    FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "Restrict write on unified_certificates to authenticated" ON unified_certificates
    FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
-- (TABLE/VIEW 키워드 없이 객체에 대해 바로 권한을 회수하여 PostgreSQL 호환성 에러 방지)
REVOKE ALL ON scholarships FROM anon;
REVOKE ALL ON scholarships_view FROM anon;

-- 오직 로그인된 세션(authenticated) 사용자만 쿼리하고 조작할 수 있도록 권한 격리
GRANT SELECT, INSERT, UPDATE, DELETE ON scholarships TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON scholarships_view TO authenticated;
