...
### 📌 9. 외부위원 심의/평가 표결 디폴트 중립화 (미선택 상태 처리)
- **최초 진입 시 자동 선택되어 보이는 UI 오류 수정 (`CommitteeExternalVote.tsx`)**
  - 원인: 최초 진입 시 찬성/동의 및 5점 평점이 디폴트로 지정되어 있어 이미 표결을 완료한 것처럼 오인되는 문제.
  - 해결: 최초 진입 시 동의·부동의·기권 및 5점 평가 모두 중립(미선택) 색상으로 표출하고, 위원이 직접 클릭한 버튼만 활성화.
  - 미선택 안건이 존재할 경우 최종 제출 시 차단 검증 로직 반영. 린트/빌드/위원회 테스트 18건 통과.

### 📌 10. 경로형 단축 URL (`/v/{shortCode}`) 시스템 도입
- **Google Forms 스타일의 경로형 단축 접근 링크 전환**
  - 기존 긴 쿼리 파라미터(`?v=9aec...`)를 `/v/muwqsxyH71FfibAAiE8YCsJF` 형태의 경로형 단축 링크로 업그레이드.
  - 링크 표시, 복사, 외부위원 안내문 전체에 `/v/` 단축주소 연동. 기존 `?v=` 주소도 100% 하위 호환 지원.
  - 만족도 조사 경로(`/sv/`)와 분리 관리 및 유효하지 않은 단축코드 안전 거부.
  - 단축 링크 헬퍼 및 단위 테스트 21건 통과 (`src/utils/committee-short-link.test.ts`, 커밋 `894c36b`).

### 📌 11. 표준 위원회 관리코드 규칙 제정, 자동 회의 차수 추출 & DB Migration 101
- **표준 위원회 관리코드 체계 구축 (`UC-ANCHOR-{연도}-{위원회코드}-{차수}`)**
  - 앵커총괄(`STEER`), 앵커기획(`PLAN`), 사업비관리(`BUDGET`), 자체평가(`EVAL`), ECC, ICC, RCC, AIDX, NURI, SEVEN 코드 제정.
  - 회의 제목(예: `제3차 앵커기획위원회 회의`)에서 회의 차수(`03`)를 자동 동적 추출하는 정규식 파서 구축 (`제2차년도` 등은 차수로 오인하지 않도록 방어). 차수 미발견 시 `XX`.
  - 외부위원 로그인 화면, 관리자 회의정보 배너, 외부위원 안내문 복사 기능에 관리코드 연동.
  - 난수형 접속 보안 식별값(Opaque public_code)은 보안 접근키로서 내부에서 분리 유지.
  - 단위 테스트 26건 통과 (`src/utils/committee-code.test.ts`, 커밋 `13306bf`).
- **Supabase DB 마이그레이션 101 적용 (`101_default_committee_public_code.sql`)**
  - `committee_meetings` 신규 생성 시 `public_code` 컬럼에 `encode(gen_random_bytes(18), 'hex')` 자동 기본값 주입 및 `NOT NULL` 제약 조건 완수.
