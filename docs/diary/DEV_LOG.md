...
- **PDF 파일 압축 시 한글 폰트/텍스트 사라짐 원천 복구 및 외부 위원 1열 전용 투표 레이아웃 전면 개편**
  - 원인 해결: `compressPdfIfNeeded`에서 `pdf.js` 로딩 시 CMap 자원(`cMapUrl`, `cMapPacked: true`) 및 표준 폰트 데이터(`standardFontDataUrl`)를 명시하여, 한글/한국어 문서 PDF가 압축 과정에서 글자가 사라지지 않도록 원천 복구.
  - UI 레이아웃 개편 1: `상정 안건 및 관련 자료`를 1열 전체 카드로 확장하고, 내부를 2열(좌: 📋 열람할 상정 의안 선택 드롭다운 + 안건 상세, 우: 📎 첨부파일 + PDF 뷰어 <iframe>)로 배치.
  - UI 레이아웃 개편 2: 그 아래에 `의결 표결 (안건별 표결)`을 1열 독립 카드로 배치하여, 의안별로 `[동의]` / `[부동의]` / `[기권]` 버튼 및 검토 의견란을 명확하게 처리.
  - UI 레이아웃 개편 3: 그 아래에 `자필 서명 (AES 암호화 보안 저장)` 블록을 1열 독립 카드로 배치 (전자서명 캔버스 패드 + 서명파일 업로드 + 지우기).
  - UI 레이아웃 개편 4: 최하단에 `[최종 의결 표결 및 서명 제출]` 버튼을 시원하게 1열 배치.
