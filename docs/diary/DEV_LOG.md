...
- **PDF 의안 심의자료 2MB 초과 시 고해상도 최적화 압축 및 안내 팝업 메시지 개편**
  - 요구 반영 1: 업로드된 PDF 파일 용량이 2MB를 초과할 경우 `ℹ️ 업로드된 PDF 용량(${origMb}MB)이 2MB를 초과하여 2MB 이하로 자동 최적화 압축을 진행합니다.` 안내 메세지 팝업 띄움.
  - 요구 반영 2: 텍스트 레이어가 뭉개지거나 깨지는 저해상도 래스터화 문제를 해결하기 위해 `pdf.js` 2.0x 고해상도 Scale, Canvas 이미지 고화질 Smoothing(`high`), JPEG Quality 0.88을 적용하여 **텍스트 글자가 깨지지 않고 아주 선명하게 보이도록 2MB 이하 자동 최적화 압축**을 구현.
  - 요구 반영 3: 압축 완료 후 `🎉 PDF 자동 최적화 압축 완료! (원래 용량: 5.2MB ➔ 선명한 최적화 용량: 1.1MB)` 완료 알림 팝업 표출.
  - UI 반영: 모달 안내 텍스트를 `* pdf 확장자만 지원 (2MB 초과 시 2MB 이하로 고해상도 자동 최적화 압축)`으로 업그레이드.
